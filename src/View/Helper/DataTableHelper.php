...
class DataTableHelper extends Helper
{
     ...
     *      e.g. [__d('sugar', 'Edit'), ['action' => 'edit', 'id' => ':id']]
     * - `paginate` boolean
     * - `sortable` boolean
     * - `filter` boolean
     * - `reduce` array
    ...
    protected function _renderFilterRow()
    {
        ...
        $actionCell = $this->templater()->format('rowCell', [
            'content' => $this->Form->button(__d('sugar', 'Filter'), ['class' => 'btn btn-default btn-xs']),
            'attrs' => $this->templater()->formatAttributes([
...
//        ]);
    ...
    protected function _renderPagination()
    {
        ...
        return $this->_View->element('Sugar.Pagination/default', [
            'counter' => ['format' => __d('sugar', 'Page {{page}} of {{pages}} . Showing {{current}} records from row {{start}} to {{end}} of {{count}} records')],
            'numbers' => [],
        ]);
    }
